...
{#if (timer.pos !== 0)}
  <g class='{timer.lane}' in:blur="{{ duration:1000 }}" out:fade="{{delay: 200, duration: 1000}}">
    ...
    <mask id="{timer.mask}">
      ...
      {#if (timer.pos - 90) < 0}
        <rect width='101' height='101' rx='-1' ry='-1' transform='rotate(-180)' fill=#000 />
      {/if}
      {#if (timer.pos - 90) > 0}
        <rect width='101' height='101' rx='-1' ry='-1' transform='rotate(-90)' fill=#fff />
      {/if}
      {#if (timer.pos - 90) > 90}
        <rect width='101' height='101' rx='-1' ry='-1' fill=#fff />
      {/if}
      {#if (timer.pos - 90) > 180}
        <rect width='101' height='101' rx='-1' ry='-1' transform='rotate(90)' fill=#fff />
      {/if}
